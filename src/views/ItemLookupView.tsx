...
const findStat = (item: ProcessedItem, statKey: string): any => {
...
export function ItemLookupView() {
    ...
    const filteredItems = useMemo(() => {
        ...
        const keywordRegex = /(category|id|mood):(?:"([^"]+)"|(\S+))/g;
        ...
        if (keywordFilters.mood) {
            const filterValue = keywordFilters.mood.replace(/_/g, ' ');
            results = results.filter(item => {
                const moodType = findStat(item, 'resolved_moodType');
                if (!moodType) return false;
                const moodName = language === 'JA' ? moodType.name_JA : moodType.name_EN;
                return typeof moodName === 'string' && moodName.toLowerCase().includes(filterValue);
            });
            results.sort((a, b) => {
                const moodA = findStat(a, 'mood') || 0;
                const moodB = findStat(b, 'mood') || 0;
                return moodB - moodA;
            });
        }
        if (nameQuery) {
