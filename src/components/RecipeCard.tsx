...
const RecipeDisplay = ({ recipe, itemMap, categoryMap, onItemClick }: Omit<RecipeCardProps, 'cultivationGroup'> & { recipe: ProcessedRecipe }) => {
    ...
    const hasCosts = (recipe.tokenCost || 0) > 0 || (recipe.observationPointCost || 0) > 0;
    return (
        <div class="space-y-3">
            <div class="flex justify-between items-center text-sm">
                <p class="text-white-400 font-semibold">{recipe.recipeTypeName} at {facilityName}</p>
                <div class="flex items-center gap-2">
                    {recipe.qualityScore !== undefined && <p class="text-gray-300 font-semibold bg-gray-800 px-2 py-1 rounded">{t('qualityScore', language)}: {recipe.qualityScore}</p>}
                    <p class="text-gray-300 font-semibold bg-gray-800 px-2 py-1 rounded">{t('area', language)}: {areaName}</p>
                </div>
            </div>
            <div class="flex items-center justify-center gap-4">
                <div class="flex flex-col gap-2 w-2/5">{recipe.materials.length > 0 ? recipe.materials.map((mat, i) => <ItemChip key={`${mat.id}-${i}`} item={!mat.isCategory ? itemMap.get(mat.id) : undefined} material={mat} count={`${mat.count}`} onClick={onItemClick} categoryMap={categoryMap} />) : <p class="text-sm text-center text-gray-400">No materials</p>}</div>
